...
The controller automatically lowercases all captured values before using them
in artifact names and labels. This means a directory named `Auth` will produce
an artifact with `app=auth`.
...
When `pathPattern` is defined, the artifacts act as a template for each matched directory.
...
- `name` (required): The name of the generated ExternalArtifact resource. It must be unique in the context
  of the ArtifactGenerator and must conform to Kubernetes resource naming conventions. Supports Go template variables if `pathPattern` is used.
